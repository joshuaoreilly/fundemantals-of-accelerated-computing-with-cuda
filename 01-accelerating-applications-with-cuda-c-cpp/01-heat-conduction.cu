...
{
  /*
  * The catch here is that thread dimensions are row and column, whereas indexing the array is j and i (j = row, i = col)
  */
  ...
  if ((i > 0 && i < ni - 1) and (j > 0 and j < nj - 1))
  {
    i00 = I2D(ni, i, j);
    im10 = I2D(ni, i-1, j);
    ip10 = I2D(ni, i+1, j);
...
int main()
{
  ...
  */
  ...
  for (istep=0; istep < nstep; istep++) {
    ...
    // checks if launch configuration is erronous
    checkCuda(cudaGetLastError());
    // check if error during kernel execution
